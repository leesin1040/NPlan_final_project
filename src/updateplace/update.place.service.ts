...
export class UpdatePlaceService {
  ...
  async uploadPlaces() {
    ...
    try {
      ...
      const placeData = await this.placeRepository.find({ where: { cat1: 'A04' } });
      placeData.map((place) => {
        place.category = cat3Mapping(place.cat3);
      });
      const data = await this.placeRepository.save(placeData);
